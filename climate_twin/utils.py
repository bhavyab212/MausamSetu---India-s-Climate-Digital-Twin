...
def ensure_data_file(filename):
    """Downloads missing data files on-the-fly from HF Datasets to bypass 1GB limits."""
    local_path = os.path.join('data', filename)
    if not os.path.exists(local_path) and HAS_HF_HUB:
        try:
            import streamlit as st
            with st.spinner(f"Downloading {filename} from cloud storage (one-time)..."):
                hf_hub_download(
                    repo_id="Sujith2005/isro-climate-data",
                    repo_type="dataset",
                    filename=filename,
                    local_dir="data",
